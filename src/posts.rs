...
pub fn post_req(path: &String, domain: &String, port: &String, body: &String, headers: &String)->[u8;16]{
...
pub fn tls_post_req(path: &String, domain: &String, port: &String, body: &String, headers: &String)->[u8;16]{
    let temp = create_post_request(path,domain,body,headers);
    let request = temp.as_bytes();
